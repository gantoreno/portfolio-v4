...
How did an **address** end up there if the mobile app sends only **coordinates**? Well, remember I mentioned **Google Cloud Platform**?

I think you already see where this is going.

That's right, the database has only **coordinates**, but the app needs **addresses**, and GCP _just so happens_ to have a service called **reverse geocoding API**, which [Google's documentation](https://developers.google.com/maps/documentation/javascript/geocoding#ReverseGeocoding) describes as the following:
