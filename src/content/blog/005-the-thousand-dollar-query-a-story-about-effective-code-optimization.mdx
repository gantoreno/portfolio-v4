...
Clearly, this is **bad**, and things get **way darker** once you realize the full extent of the problem &ndash; we weren't even at a 10% of the intended usage! Just by how the system was built (considering that each client would have their **own separate database**) we were able to run some numbers to try and predict how the billing would **scale** as the user-base grows, and let me tell you, _it's way worse than you could imagine_. Just to give you an idea, here's a _very_ high-level approximation of these numbers I came up with at the time:
...
And these facts hold true for the companies that this system targets as well. Two weeks later, we were at **30% of the usage**, and it kept consistently decreasing even more and more &ndash; by the end of the month, we were charged with **only $183** or so, meaning, we had successfully saved **~86% of past month's bill**.
