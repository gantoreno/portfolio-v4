...
This is the story of how we tamed [Google Cloud Platform](https://cloud.google.com/) and significantly reduced costs by **thinking**, **testing**, **iterating** and using the power of, guess what? **Math**.
...
Turns out, **it wasn't the case**. The request at load-time would bring **all data altogether**, rendering the pagination **completely useless** (making it merely a visual thing), _but it doesn't end there_ &ndash; remember I purposefully quoted the so-called "real-time" part? One would _assume_ that, after the UI loads (along with the entire list entries), newer events would just slot a new entry into state, since past events have **already been loaded**.
...
Let's see, it has a _name_, a _type_, an _address_... Hold on, an **address**? Didn't you just say this in the previous section?
...
I know, _what the hell?_ Breaking down the equation we have:

- $R$ as the **total amount of requests** made in a day.
- $C$ as the **number of companies actively using the system**, by that time, it was **just us** (1).
...
Alright, we had to come up with something, what are our options? The first solution that came to mind was the most obvious &ndash; **fix the pagination** and the **notification system**. There's no discussion, this was a **must**, and looking into other options without considering this would've been pure negligence, however, there were some _slight_ problems:

- The frontend guy gave a **very broad estimate**, he said he needed to _"rewire"_ that component's **inner workings**, and he would have to spend some time syncing up with the backend developer to **refactor** the endpoint in order to support **paginated requests** (I know, you're probably <Underline>screaming</Underline> at this. I was too).
...
and speed detection would start miscalculating everything &ndash; if it took <Underline>3 decimal places</Underline> to make a <Underline>100 meter difference</Underline>, in the eyes
of the speed measurements the person would remain **still** (this is, 0m/s) for
...
100m/s before going back to **being still again**:
...
And the best part? **It's just a query change**! This filter can be shoved inside our SQL statement by adding the third term `distance` as the equation result, paired with a `HAVING` clause that sets a **clear threshold** for the search radius we're going after &ndash; this would end up looking something like this:
...
One thing is certain, though &ndash; had we not acted **quickly** and tried our best to **craft** a **reasonable solution**, this issue would've caused **pretty serious damage** the project, in a **way deeper** level than anyone would've anticipated. Thankfully, <Underline>some equations</Underline> and a _bit_ of <Underline>abstract thinking</Underline> thinking were the nails in the coffin of this _awful_ situation.
